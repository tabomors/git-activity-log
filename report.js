...
const log = (schema, proccessValue = (k, v) => v) => new Promise((resolve, reject) => {
  ...
  exec(command(formatParams), (err, stdout) => {
    if (err) reject(err);
      ...
      const listOfLogs = stdout
        ...
        .map(line => line.split(PARAMS_SEPARATOR).reduce((obj, value, idx) => {
          const key = keys[idx];
...
const SCHEMA = {
  date: '%ad', message: '%B', refs: '%d', author: '%an',
};
